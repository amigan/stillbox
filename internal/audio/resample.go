...
type Resampler[F floatConstraint] struct {
	FromRate int // The original audio sample rate.
	ToRate   int // The resampled audio sample rate.
	Channels int // The amount of channels.
}

func NewResampler[F floatConstraint](channels, inputRate, outputRate int) (*Resampler[F], error) {
	if channels < 1 {
...
func (resampler *Resampler[F]) ResampleFloat(data []F) []F {
	if len(data) == 0 {
		return nil
	}
	...
	*/
	...
	// Split channels
	channels := make([][]F, resampler.Channels)
	...
	resampled := make(
		[]F,
		int((F(len(data))/F(resampler.FromRate))*F(resampler.ToRate)),
	)

	// Resample channels
	resampledData := make([][]F, len(channels))
...
func (resampler *Resampler[F]) ResampleInt16(data []int16) (resampledi16 []int16) {
	// Convert the data to float64
	f64data := make([]F, len(data))
	for i := 0; i < len(data); i++ {
		f64data[i] = F(data[i]) / F(0x7FFF)
	}
	// Resample
	resampledf64 := resampler.ResampleFloat(f64data)

	// Convert back to int16
	resampledi16 = make([]int16, len(resampledf64))
	for i := 0; i < len(resampledf64); i++ {
		resampledi16[i] = int16(resampledf64[i] * F(0x7FFF))
	}
	return
}

func (resampler *Resampler[F]) resampleChannelData(data []F) []F {
	// Need at least 16 samples to resample a channel
	if len(data) <= 16 {
		return make([]F, len(data))
	}
	...
	channelFrom := F(resampler.FromRate) / F(resampler.Channels)
	channelTo := F(resampler.ToRate) / F(resampler.Channels)
	step := channelFrom / channelTo

	output := []F{}

	// Resample each position from x0
	for x := step; x < F(availSamples); x += step {
		xi0 := F(uint64(x))
		xi := []F{xi0, xi0 + 1, xi0 + 2, xi0 + 3}
		yi0 := uint64(xi0)
		yi := []F{
			F(data[yi0]),
			F(data[yi0+1]),
		...
		output = append(output, yo[0]/F(0x7FFF))
	}
	return output
}

func spline[F floatConstraint](xi, yi, xo, yo []F) (err error) {
	if len(xi) != 4 {
...
func splineZ0[F floatConstraint](m1, h0, x0, x1, y0, y1, x F) F {
	v0 := F(0.0)
...
func splineZ1[F floatConstraint](m1, m2, h1, x1, x2, y1, y2, x F) F {
...
func splineZ2[F floatConstraint](m2, h2, x2, x3, y2, y3, x F) F {
...
func splineM1[F floatConstraint](c1, c2, u1, l2 F) F {
	return (c1/u1 - c2/2) / (2/u1 - l2/2)
}

func splineM2[F floatConstraint](c1, c2, u1, l2 F) F {
	return (c1/2 - c2/l2) / (u1/2 - 2/l2)
}

func splineC1[F floatConstraint](yi []F, h0, h1 F) F {
	y0, y1, y2, _ := yi[0], yi[1], yi[2], yi[3]
	return 6.0 / (h0 + h1) * ((y2-y1)/h1 - (y1-y0)/h0)
}

func splineC2[F floatConstraint](yi []F, h1, h2 F) F {
	_, y1, y2, y3 := yi[0], yi[1], yi[2], yi[3]
	return 6.0 / (h1 + h2) * ((y3-y2)/h2 - (y2-y1)/h1)
}

func splineLU[F floatConstraint](xi []F) (h0, h1, h2, l1, u1, l2, u2 F) {
	x0, x1, x2, x3 := xi[0], xi[1], xi[2], xi[3]
