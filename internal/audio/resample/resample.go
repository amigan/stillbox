/*
	...
	Modified to specify separate input and output formats by Daniel Ponte.
*/
...
type Resampler struct {
	...
	iframeSize  int       // frame size in bytes
	oframeSize  int       // frame size in bytes
...
func New(writer io.Writer, inputRate, outputRate float64, channels, iformat, oformat, quality int) (*Resampler, error) {
	var err error
	var isize, osize int
	if writer == nil {
	...
	ioSpec := C.soxr_io_spec(C.soxr_datatype_t(iformat), C.soxr_datatype_t(oformat))
...
func (r *Resampler) Write(p []byte) (int, error) {
	...
	framesIn := len(p) / r.iframeSize / r.channels
	if framesIn == 0 {
	...
	dataIn := C.CBytes(p)
	dataOut := C.malloc(C.size_t(framesOut * r.channels * r.oframeSize))
	var soxErr C.soxr_error_t
	...
	_, err = r.destination.Write(C.GoBytes(dataOut, C.int(int(done)*r.channels*r.oframeSize)))
