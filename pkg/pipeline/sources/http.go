...
func (h *RdioHTTP) routeCallUpload(w http.ResponseWriter, r *http.Request) {
	...
	log.Debug().Int("system", cur.System).Int("tgid", cur.Talkgroup).Str("duration", call.Duration.Duration().String()).Str("sub", submitter.Username).Bool("filtered", call.Filtered()).Msg("ingested")
